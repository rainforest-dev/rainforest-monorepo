...
docs = [{
    "slug": "sdf", "machines": ["laptop", "rainforest-angible"], "lifecycle": "active",
...
p = publish_project_assignment(docs, machine="rainforest-angible", now_ts=1)
...
check "file is named for the machine" \
  "$(printf '%s' "$OUT" | head -1)" "projects.rainforest-angible.json"

echo "== carries the join columns =="
check "keeps the machines list" \
  "$(printf '%s' "$OUT" | grep -c '"rainforest-angible"')" "2"
