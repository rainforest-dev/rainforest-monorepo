...
module.exports=${proxy('window.React', '{jsx:jsx,jsxs:jsx,jsxDEV:jsx,Fragment:undefined}')};`,
  ...
  // .storybook/preview imports the Tailwind v4 source stylesheet (`@import 'tailwindcss'`), which esbuild cannot resolve; the bundle css already ships every class.
  const cssEmpty = {
    name: 'css-empty',
    setup(b) {
      b.onLoad({ filter: /\.css$/ }, () => ({ contents: '', loader: 'empty' }));
    },
  };
  try {
    await build({
      ...
      plugins: [reactGlobal, dsShim, stubEmpty, cssEmpty],
