...
function FilterCombobox({
  ...
  const selectedLabel = value
    ? (options.find((o) => String(o.id) === value)?.name ?? value)
    : null;
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger
        ...
        className="border-input bg-background hover:bg-accent hover:text-accent-foreground shadow-xs inline-flex h-8 w-full items-center justify-between rounded-md border px-3 py-1 text-sm sm:w-40"
      >
          ...
          <CommandList>
            ...
            {options.map((o) => (
              <CommandItem
                key={o.id}
                value={String(o.id)}
                onSelect={() => handleSelect(String(o.id))}
              >
                <Check
                  className={cn(
                    'mr-2 h-4 w-4',
...
function FilterBarInner({ filters, platforms }: Props) {
    ...
    const id = setTimeout(async () => {
      ...
      try {
        const res = await fetch(
          `/api/books/search?q=${encodeURIComponent(localQ)}`,
        );
        setSuggestions(
          ((await res.json()) as { results: SearchResult[] }).results ?? [],
        );
      } catch {
        setSuggestions([]);
      }
  ...
  const platformLabel = activePlatform
    ? (platforms.find((p) => p.key === activePlatform)?.name ?? activePlatform)
    : null;
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center">
        ...
        {activePlatform && (
          <Select
            value={activeDelivered ?? 'all'}
            onValueChange={(v) =>
              updateParam('delivered', v === 'all' ? null : v)
            }
          >
            <SelectTrigger
        ...
        <div className="flex items-center gap-2 sm:ml-auto">
          <Select
            ...
            onValueChange={(v) => {
              updateParam('groupBy', v === 'none' ? null : v);
          ...
          <Select
            value={activeSortBy ?? 'title'}
            onValueChange={(v) =>
              updateParam('sortBy', v === 'title' ? null : v)
            }
          >
          ...
          <button
            ...
            onClick={() =>
              updateParam('sortDir', activeSortDir === 'desc' ? null : 'desc')
            }
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {platformLabel && (
            <FilterChip
              ...
              onRemove={() => {
                updateParam('platform', null);
                updateParam('delivered', null);
              }}
            />
          )}
          <button
