    ...
    <div className="border-border bg-card text-card-foreground rounded-xl border p-6">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-4">
        ...
        <button
          type="button"
          onClick={handlePaste}
          className="border-primary/50 text-foreground hover:bg-primary/10 focus-visible:ring-ring h-8 rounded-md border bg-transparent px-3 text-sm font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2"
        >
          paste a phrase
        </button>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2.5">
        {cells.map((cell, index) => {
          ...
          return (
            <label
              key={index}
              className={`bg-muted/40 focus-within:ring-ring flex h-11 items-center gap-2 rounded-lg border px-3 transition-colors focus-within:ring-2 ${
                invalid
