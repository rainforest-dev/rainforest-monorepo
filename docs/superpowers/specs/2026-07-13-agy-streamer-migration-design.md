...
- **At least three distinct interactive-prompt shapes observed** during testing, all sharing a common visual structure (a `>` cursor marking the selected option, sibling options on following indented lines, an arrow-key navigation hint footer) — but **numbering is not consistent across shapes**, so the parser can't assume `1./2./3.` prefixes:
  1. Workspace trust (first use of a new directory): `"Do you trust the contents of this project?"` — 2 **unnumbered** options (`> Yes, I trust this folder` / `  No, exit`), footer `"↑/↓ Navigate · enter Confirm"`.
  2. Command approval: `"Requesting permission for: <command>"` / `"Do you want to proceed?"` — 4 **numbered** options (`1.`–`4.`: yes-once / always-this-conversation / always-persisted / no), footer `"↑/↓ Navigate · tab Amend · ctrl+g edit/expand command"` then `"esc to cancel"`.
  3. File access/creation: `"Allow creation of this file?"` or `"Allow access to this file?"` — 3 **numbered** options (yes / yes-always-non-workspace / no), similar footer.
...
- **Generic menu-prompt detector**: look for a line starting with `>` followed by non-empty text (the cursor-selected option — numbered or not), followed by one or more sibling lines indented to the same column (additional options, each optionally prefixed `N.`), and a trailing navigation-hint line containing `"Navigate"` or `"Confirm"` or `"esc to cancel"`. Deliberately does **not** require numbered options (workspace-trust isn't numbered) — this catches all 3 confirmed shapes *and* an unfamiliar 4th shape (e.g. a subagent prompt) without hardcoding exact wording. When sending a decision back, if the matched option line had a `N.` prefix, send `"N\n"`; otherwise send the arrow-key sequence to move the cursor to the chosen option followed by Enter (`↓` repeated *k* times + `\r`, since unnumbered options can only be selected positionally).
